...
#Need to create a function to use the area_list and go through each file in the data folder
...
for(i in 1:length(all_years)){
  print(i)
  for (j in 1:length(all_areas)){
    print(j)
    year_data <- vector("list", 17)
    if (is.null(all_areas[[j]][[2]])) {
      year_data[j] <- extract_tables(file = all_years[i], page = 1, area = all_areas[[j]][1])
    } else {
      year_data[j] <- extract_tables(file = all_years[i], page = 2, area = all_areas[[j]][2])
    }
  }
