...
faculty_data <- vector("list", length(all_years))
...
#First extract the area
#Next subset to get the page temp[[2]]
#NExt convert to data frame temp_df <- as.data.frame(tmep[[2]])
#Add a year column temp_df$V6 <- YEAR
#Need to clean up the randomn lettering and work on naming
#Could ignore the column of names and write it in from scratch instead
#consider making Full time, part time, and FTE as columns of the type of staff


for(i in 1:length(all_years)){
  
  extracted <- extract_areas(file = all_years[i])
  extracted_df <- as.data.frame(extracted[[2]])
  extracted_df$Year <- all_years[i]
  
  datalist[[i]] < extracted_df
}

combined_data <- do.call(cbind, datalist)
